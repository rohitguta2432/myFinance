        ...
        <div className={`rounded-xl border ${bg} overflow-hidden`}>
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-500 ml-1">{items.length} items</span>
                </div>
        ...
        <div className="rounded-xl border bg-blue-500/5 border-blue-500/15 overflow-hidden">
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold tracking-wide text-blue-400">Step-by-Step Guide</span>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-surface-dark/80 p-5 space-y-4 hover:border-white/10 transition-all">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                            {/* Category badge */}
                            <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wider ${catConfig.bg} ${catConfig.text} border ${catConfig.border}`}>
                                {action.category}
                            </span>
                            {/* Pillar */}
                            <div className="flex items-center gap-1">
                                <PillarIcon size={11} className="text-slate-500" />
                                <span className="text-[10px] text-slate-500 font-medium">{action.pillar}</span>
                            </div>
                            {/* Scenario */}
                            <span className="text-[10px] text-slate-600 font-mono">{action.scenario}</span>
                        </div>
                        <h3 className="text-sm font-bold text-white leading-snug">{action.title}</h3>
                        <p className="text-xs text-slate-400 leading-relaxed">{action.subtitle}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-primary/10 border border-primary/20">
                    ...
                    <span className="text-[11px] font-bold text-primary">Impact: {action.fmt ? action.fmt(action.impactAmount) : `₹${action.impactAmount.toLocaleString('en-IN')}`}</span>
                </div>
                ...
                {action.feasibilityFactor < 1 && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        ...
                        <span className="text-[11px] font-bold text-amber-400">Build EF First</span>
                    </div>
                )}
            </div>
            ...
            <div className="max-w-3xl mx-auto px-4 pt-10 space-y-6">
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-white tracking-tight">Action Plan</h1>
                            <p className="text-xs text-slate-500">{fyLabel} · {count} action{count !== 1 ? 's' : ''} for your profile</p>
                        </div>
                    </div>
                    ...
                    {count === 0 && (
                        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 text-center space-y-2">
                            ...
                            <p className="text-xs text-slate-400">No priority actions detected for your profile. You're doing great!</p>
                        </div>
                    )}
