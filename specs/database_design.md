...
## Overview
This document outlines the PostgreSQL database schema for the MyFinance application. The design is normalized to support functionality from the 6-step assessment flow.
...
erDiagram
    USERS ||--|| PROFILES : has
    USERS ||--|{ INCOMES : earns
    USERS ||--|{ EXPENSES : spends
    USERS ||--|{ ASSETS : owns
    USERS ||--|{ LIABILITIES : owes
    USERS ||--|{ GOALS : plans
    USERS ||--|{ INSURANCES : covers
    USERS ||--|| TAX_PLANNING : manages
    ...
    PROFILES {
        ...
        uuid user_id FK
        int age
        string city_tier "METRO, TIER_1, TIER_2"
        string marital_status "SINGLE, MARRIED"
        int dependents
        string employment_type "SALARIED, SELF_EMPLOYED"
        string residency_status "RESIDENT, NRI"
        float risk_score
    ...
    ASSETS {
        uuid id PK
        uuid user_id FK
        string type "REAL_ESTATE, EQUITY, GOLD, CASH"
        string name
        decimal value
    }
    ...
    LIABILITIES {
        uuid id PK
        uuid user_id FK
        string type "HOME_LOAN, CAR_LOAN, PERSONAL_LOAN"
        string name
    ...
    GOALS {
        uuid id PK
        uuid user_id FK
        string type "HOME, CAR, RETIREMENT"
        string name
    ...
    TAX_PLANNING {
        uuid id PK
         ...
         decimal epf_vpf
         decimal ppf_elss
         decimal tuition_fees
         decimal medical_insurance_80d
         string selected_regime "OLD, NEW"
    }
```
...
CREATE TABLE tax_planning (
    ...
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_tax_planning_user_id UNIQUE (user_id)
);
```
