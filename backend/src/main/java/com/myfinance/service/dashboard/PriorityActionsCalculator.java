    ...
    public PriorityActionsDTO calculate(UserFinancialData d, RawDataDTO rawData) {
        ...
        if (emergencyMonths < 6) {
            ...
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .description(String.format(
                            "You have %.1f months of expenses saved. Target: 6 months (%s).",
                            emergencyMonths, fmt(6 * monthlyExpenses)))
                    .impactLabel("HIGH")
                    .urgencyLabel(emergencyMonths < 3 ? "CRITICAL" : "HIGH")
                    .impact(gap)
                    .urgency(emergencyMonths < 3 ? 3.0 : 2.0)
                    .howTo("Set up auto-sweep RD/FD for surplus each month until you reach 6 months of expenses.")
                    .priorityScore(emergencyMonths < 3 ? 95.0 : 85.0)
                    .category("survival")
                    .build());
        }
        ...
        if (lifeCoverRatio < 1.0) {
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .description(String.format(
                            "Your life cover is %.0f%% of required. Gap: %s.",
                            lifeCoverRatio * 100, fmt(rawData.getRequiredCover() - rawData.getExistingTermCover())))
                    .impactLabel("HIGH")
                    .urgencyLabel(lifeCoverRatio < 0.5 ? "CRITICAL" : "HIGH")
                    .impact(rawData.getRequiredCover() - rawData.getExistingTermCover())
                    .urgency(lifeCoverRatio < 0.5 ? 3.0 : 2.0)
                    .howTo("Get a term plan online for the gap amount. Compare premiums on PolicyBazaar.")
                    .priorityScore(lifeCoverRatio < 0.5 ? 92.0 : 80.0)
                    .category("protection")
                    .build());
        }
        ...
        if (rawData.getExistingHealthCover() < rawData.getHealthBenchmark()) {
            ...
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .description(String.format(
                            "Health cover gap: %s below benchmark of %s.", fmt(gap), fmt(rawData.getHealthBenchmark())))
                    .impactLabel("HIGH")
                    .urgencyLabel("HIGH")
                    .impact(gap)
                    .urgency(2.0)
        ...
        if (emiRatio > 40) {
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .impactLabel("HIGH")
                    .urgencyLabel("HIGH")
                    .impact((emiRatio - 30) * monthlyIncome / 100)
                    .urgency(2.0)
        ...
        if (savingsRate < 20) {
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .impactLabel("HIGH")
                    .urgencyLabel(savingsRate < 10 ? "CRITICAL" : "MEDIUM")
                    .impact((20 - savingsRate) * monthlyIncome / 100)
                    .urgency(savingsRate < 10 ? 2.0 : 1.0)
                    .howTo("Automate SIP on salary day. Cut 2-3 discretionary expenses.")
                    .priorityScore(savingsRate < 10 ? 87.0 : 70.0)
                    .category("wealth")
                    .build());
        }
        ...
        if (rawData.getEquityPct() < rawData.getTargetEquityPct() * 0.5) {
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .description(String.format(
                            "Equity is %.0f%% vs target %.0f%%. Start SIP in index/flexicap funds.",
                            rawData.getEquityPct(), rawData.getTargetEquityPct()))
                    .impactLabel("MEDIUM")
                    .urgencyLabel("MEDIUM")
                    .impact(d.getTotalAssets() * (rawData.getTargetEquityPct() - rawData.getEquityPct()) / 100)
                    .urgency(1.0)
        ...
        if (rawData.getNwMultiplier() < rawData.getBenchmarkMultiplier() * 0.5) {
            actions.add(PriorityActionItemDTO.builder()
                    ...
                    .description(String.format(
                            "Net worth is %.1fx income vs %.1fx benchmark. Consider NPS/PPF.",
                            rawData.getNwMultiplier(), rawData.getBenchmarkMultiplier()))
                    .impactLabel("HIGH")
                    .urgencyLabel("MEDIUM")
                    .impact((rawData.getBenchmarkMultiplier() - rawData.getNwMultiplier()) * d.getAnnualIncome())
                    .urgency(1.0)
