...
public class RedFlagsCalculator {
    ...
    public RedFlagsDTO calculate(UserFinancialData d, RawDataDTO rawData) {
        ...
        if (d.getLiquidAssets() <= 0) {
            flags.add(flag("no_emergency", "🚨", "No Emergency Fund",
                    ...
                    "critical", "survival", 100, 3.0));
        } else if (emergencyMonths < 3) {
            double gap = (6 * monthlyExpenses) - d.getLiquidAssets();
            flags.add(flag("low_emergency", "⚠️", "Critically Low Emergency Fund",
        ...
        if (emiRatio > 50) {
            flags.add(flag("extreme_emi", "🚨", "Extreme EMI Burden",
                    ...
                    "critical", "debt", 93, 3.0));
        } else if (emiRatio > 40) {
            flags.add(flag("high_emi", "⚠️", "High EMI Burden",
        ...
        // 5. Negative savings
        if (savingsRate < 0) {
            flags.add(flag("negative_savings", "🚨", "Spending More Than Earning",
                    ...
                    "critical", "wealth", 97, 3.0));
        } else if (savingsRate < 10) {
            flags.add(flag("low_savings", "⚠️", "Very Low Savings Rate",
        ...
        return RedFlagsDTO.builder().flags(new ArrayList<>(top)).totalCount(flags.size()).build();
    }

    private RedFlagItemDTO flag(String id, String icon, String title, String explanation, String action,
                                String severity, String category, double impact, double urgency) {
        return RedFlagItemDTO.builder().id(id).icon(icon).title(title)
                .description(explanation).explanation(explanation).action(action)
                .severity(severity).category(category).impact(impact).urgency(urgency).build();
    }
}
