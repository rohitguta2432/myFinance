...
public class HealthScoreCalculator {
    ...
    public HealthScoreDTO calculate(UserFinancialData d) {
        ...
        double goalCosts = d.getGoals().stream()
                ...
                .mapToDouble(g -> g.getCurrentCost() != null ? g.getCurrentCost() : 0).sum();
        ...
        double currentLiab12m = d.getLiabilities().stream().mapToDouble(l -> {
        ...
        Optional<Goal> retGoal = d.getGoals().stream()
                .filter(g -> "retirement".equals(g.getGoalType())).findFirst();
        ...
        double ageWealthScore = Math.min(5, Math.max(0, benchmarkMultiplier > 0 ? (nwMultiplier / benchmarkMultiplier) * 5 : 0));
        ...
        List<PillarDTO> pillars = List.of(
                buildPillar("survival", "Survival", survivalScore, 25, "🛡️", "#ef4444",
                        emergencyFundMonths < 6 ? String.format("%.1f months", emergencyFundMonths) : String.format("%.1f months buffer", emergencyFundMonths),
                        emergencyFundMonths < 6 ? "Emergency fund critically low" : "Emergency fund is healthy"),
                buildPillar("protection", "Protection", protectionScore, 20, "🔒", "#8b5cf6",
                        String.format("%.0f%% adequate", lifeCoverRatio * 100),
                        lifeCoverRatio < 0.5 ? "Severely under-insured" : lifeCoverRatio < 1 ? "Partially covered" : "Adequately insured"),
                buildPillar("debt", "Debt", debtScore, 20, "💳", "#f59e0b",
                        monthlyEMI > 0 ? String.format("%.0f%% DTI", emiToIncomeRatio) : "No debt",
                        emiToIncomeRatio > 40 ? "EMI burden is high" : emiToIncomeRatio > 30 ? "EMI burden needs monitoring" : "EMI burden manageable"),
                buildPillar("wealth", "Wealth", wealthScore, 20, "📈", "#3b82f6",
                        String.format("%.0f%% savings", savingsRate),
                        equityPct < targetEquityPct * 0.5 ? "Equity exposure gap" : "Savings on track"),
                buildPillar("retirement", "Retirement", retirementScore, 15, "🏖️", "#06b6d4",
                        String.format("%.1fx multiplier", nwMultiplier),
                        nwMultiplier < benchmarkMultiplier * 0.5 ? String.format("Retiring %d years late", (int) Math.max(0, Math.round((benchmarkMultiplier - nwMultiplier) * 2))) : "On track for retirement")
        );

        // Sort by deficit DESC
        Map<String, Integer> priorityOrder = Map.of("survival", 0, "protection", 1, "debt", 2, "wealth", 3, "retirement", 4);
        ...
        String scoreLabel; String scoreLabelColor;
        if (totalScore <= 40) { scoreLabel = "NEEDS ATTENTION"; scoreLabelColor = "#ef4444"; }
        else if (totalScore <= 65) { scoreLabel = "FAIR"; scoreLabelColor = "#f59e0b"; }
        else if (totalScore <= 80) { scoreLabel = "GOOD"; scoreLabelColor = "#3b82f6"; }
        else { scoreLabel = "EXCELLENT"; scoreLabelColor = "#0DF259"; }

        RawDataDTO rawData = RawDataDTO.builder()
                .liquidAssets(liquidAssets).monthlyExpenses(monthlyExpenses).monthlyIncome(monthlyIncome)
                .annualIncome(annualIncome).monthlyEMI(monthlyEMI).emergencyFundMonths(emergencyFundMonths)
                .totalAssets(totalAssets).totalLiabilities(totalLiabilities).netWorth(netWorth)
                .existingTermCover(existingTermCover).existingHealthCover(existingHealthCover)
                .requiredCover(requiredCover).healthBenchmark(healthBenchmark)
                .emiToIncomeRatio(emiToIncomeRatio).dti(dti * 100).savingsRate(savingsRate)
                .equityPct(equityPct).targetEquityPct(targetEquityPct)
                .nwMultiplier(nwMultiplier).benchmarkMultiplier(benchmarkMultiplier)
                .fiRatio(fiRatio).retirementContribution(retirementContribution)
                .retirementAge(retirementAge).age(age).lifeCoverRatio(lifeCoverRatio)
                .monthlySurplus(monthlySavings).grossIncome(monthlyIncome)
                .dscr(dscr).lifeScore(lifeScore).healthScore(healthScore)
        ...
        return HealthScoreDTO.builder()
                .totalScore(totalScore).scoreLabel(scoreLabel).scoreLabelColor(scoreLabelColor)
                .pillars(pillars).sortedPillars(sorted).mostCritical(sorted.get(0))
                .rawData(rawData).build();
    }

    private PillarDTO buildPillar(String id, String name, double score, int max, String icon, String color, String shortI, String longI) {
        return PillarDTO.builder().id(id).name(name).score(score).maxScore(max)
                .deficit(max - score).icon(icon).color(color).shortInsight(shortI).longInsight(longI).build();
    }
