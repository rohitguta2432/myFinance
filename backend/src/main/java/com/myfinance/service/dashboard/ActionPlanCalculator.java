    ...
    public ActionPlanDTO calculate(UserFinancialData d, RawDataDTO rawData) {
        ...
        if (emergencyGap > 0) {
            ...
            actions.add(ActionItemDTO.builder()
                    .id("A1").icon("🛡️").title("Build Emergency Fund")
                    .description(String.format("Create a %s emergency buffer.", fmt(targetEmergency)))
                    .impact(emergencyMonths < 3 ? "HIGH" : "MEDIUM").urgency(emergencyMonths < 3 ? "CRITICAL" : "HIGH")
                    .feasibility(monthlySIP < monthlyIncome * 0.1 ? "EASY" : "MODERATE")
                    .priorityScore(emergencyMonths < 3 ? 95.0 : 82.0)
                    .whatToDo(String.format("Start a monthly SIP of %s into a liquid fund or high-yield savings.", fmt(monthlySIP)))
        ...
        if (lifeGap > 0) {
            actions.add(ActionItemDTO.builder()
                    .id("A2").icon("🔒").title("Get Adequate Life Insurance")
                    .description(String.format("Close %s life cover gap.", fmt(lifeGap)))
                    .impact("HIGH").urgency(rawData.getLifeCoverRatio() < 0.5 ? "CRITICAL" : "HIGH")
                    .feasibility("EASY").priorityScore(90.0)
                    .whatToDo(String.format("Buy a term plan for %s. Online plans cost ₹600–1,000/month for ₹1Cr.", fmt(lifeGap)))
        ...
        if (healthGap > 0) {
            actions.add(ActionItemDTO.builder()
                    .id("A3").icon("🏥").title("Increase Health Cover")
                    .description(String.format("Health cover gap of %s.", fmt(healthGap)))
                    .impact("HIGH").urgency("HIGH").feasibility("EASY").priorityScore(85.0)
                    .whatToDo("Buy a family floater or super top-up plan.")
                    .whyItMatters("Medical inflation is 14%. One surgery can cost ₹5–15 lakh.")
                    .expectedOutcome(String.format("Health cover reaches benchmark of %s.", fmt(rawData.getHealthBenchmark())))
                    .build());
        }
        ...
        if (emiRatio > 30) {
            actions.add(ActionItemDTO.builder()
                    .id("A4").icon("💳").title("Reduce EMI Burden")
                    .description(String.format("EMI-to-income at %.0f%%. Target: below 30%%.", emiRatio))
                    .impact("HIGH").urgency(emiRatio > 40 ? "CRITICAL" : "HIGH")
                    .feasibility("MODERATE").priorityScore(emiRatio > 40 ? 88.0 : 72.0)
        ...
        if (savingsRate < 20) {
            ...
            actions.add(ActionItemDTO.builder()
                    .id("A5").icon("📈").title("Increase Savings Rate")
                    .description(String.format("Savings rate %.0f%%. Target: 20%%+.", savingsRate))
                    .impact("HIGH").urgency(savingsRate < 10 ? "CRITICAL" : "MEDIUM")
                    .feasibility(targetAdditional < monthlyIncome * 0.05 ? "EASY" : "MODERATE")
                    .priorityScore(savingsRate < 10 ? 86.0 : 68.0)
                    .whatToDo(String.format("Automate %s/month into a SIP before spending.", fmt(Math.max(0, targetAdditional))))
        ...
        if (d.getEquityPct() < rawData.getTargetEquityPct() * 0.5 && d.getMonthlySavings() > 0) {
            actions.add(ActionItemDTO.builder()
                    .id("A6").icon("📊").title("Start Equity SIPs")
                    .description(String.format("Equity at %.0f%% vs target %.0f%%.", d.getEquityPct(), rawData.getTargetEquityPct()))
                    .impact("MEDIUM").urgency("MEDIUM").feasibility("EASY").priorityScore(62.0)
                    ...
                    .expectedOutcome(String.format("Equity allocation reaches %.0f%% in 2–3 years.", rawData.getTargetEquityPct()))
                    .build());
        }
        ...
        if (rawData.getNwMultiplier() < rawData.getBenchmarkMultiplier() * 0.5) {
            actions.add(ActionItemDTO.builder()
                    .id("A7").icon("🏖️").title("Accelerate Retirement Savings")
                    .description(String.format("Wealth multiplier %.1fx vs %.1fx benchmark.", rawData.getNwMultiplier(), rawData.getBenchmarkMultiplier()))
                    .impact("HIGH").urgency("MEDIUM").feasibility("MODERATE").priorityScore(70.0)
