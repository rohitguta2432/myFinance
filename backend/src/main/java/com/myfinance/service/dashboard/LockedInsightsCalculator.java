    ...
    public LockedInsightsDTO calculate(UserFinancialData d, RawDataDTO rawData) {
        ...
        if (d.getAnnualIncome() > 500000) {
            ...
            all.add(insight(
                    ...
                    String.format("Potential tax saving of %s through 80C/NPS optimisation.", fmt(potential)),
                    "tax",
                    85));
        }
        ...
        if (rawData.getLifeCoverRatio() < 1) {
            all.add(insight(
                    ...
                    "Your life insurance cover needs attention. See detailed analysis.",
                    "insurance",
                    80));
        }
        ...
        if (emiRatio > 30) {
            all.add(insight(
                    ...
                    "High EMI burden detected. Balance transfer could save interest.",
                    "debt",
                    78));
        }
        ...
        if (emergencyMonths < 6) {
            all.add(insight(
                    ...
                    String.format("%.1f months buffer. Build to 6 months for safety.", emergencyMonths),
                    "savings",
                    82));
        }
        ...
        if (rawData.getNwMultiplier() < rawData.getBenchmarkMultiplier()) {
            all.add(insight(
                    ...
                    "Your wealth accumulation is behind the age benchmark.",
                    "retirement",
                    70));
        }
        ...
        if (d.getMonthlySavings() > 0 && d.getEquityPct() < 30) {
            all.add(insight(
                    ...
                    "Start Equity SIP",
                    String.format(
                            "You have %s monthly surplus. A SIP could accelerate wealth.", fmt(d.getMonthlySavings())),
                    "wealth",
                    68));
        }
        ...
        if (d.getExistingHealthCover() < 1000000) {
            all.add(insight(
                    ...
                    "🏥",
                    "Health Cover Review",
                    "Health insurance below recommended ₹10L benchmark.",
                    "insurance",
                    75));
        }
