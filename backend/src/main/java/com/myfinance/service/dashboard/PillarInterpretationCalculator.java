...
public class PillarInterpretationCalculator {
    ...
    private PillarInterpretationDTO computeSurvival(List<PillarDTO> pillars, RawDataDTO raw) {
        ...
        if (liquid == 0 || score <= 10) {
            ...
            return PillarInterpretationDTO.builder()
                    ...
                    .dscrOverride(false).equityOverride(false).build();
        }
        ...
        if (score <= 17) {
            ...
            return PillarInterpretationDTO.builder()
                    .tier("warn").status("WARNING")
                    .text("You have " + monthsDisplay + " of runway — below the 6-month safety net for your income profile.")
                    .action("Increase monthly contributions to your emergency fund. Target: ₹" + monthlyTarget + "/month to reach 6-month buffer within a year.")
                    .dscrOverride(false).equityOverride(false).build();
        }
    ...
    private PillarInterpretationDTO computeProtection(List<PillarDTO> pillars, RawDataDTO raw) {
        ...
        if (lifeScoreVal < 4 && healthScoreVal < 4) {
            return PillarInterpretationDTO.builder()
                    .tier("critical").status("CRITICAL")
                    .text("Your family has ₹" + formatCr(Math.max(0, required - existingTerm)) + " Cr in uninsured exposure — we found the lowest-cost way to close this gap.")
        ...
        if (lifeScoreVal >= 8 && healthScoreVal < 4) {
            return PillarInterpretationDTO.builder()
                    .tier("warn").status("WARNING")
                    .text("You are ₹" + formatLakh(Math.max(0, cityHealthBenchmark - existingHealth)) + " Lakh under-insured on health cover — one hospitalisation could cost you this.")
    ...
    private PillarInterpretationDTO computeDebt(List<PillarDTO> pillars, RawDataDTO raw) {
        ...
        if (dscr < 1.0) {
            return PillarInterpretationDTO.builder()
                    .tier("critical").status("CRITICAL")
                    .text("₹" + formatRupees(Math.round(monthlyEMI)) + "/month is trapped in EMIs — debt restructuring could free this up within 90 days.")
                    ...
                    .dscrOverride(true).equityOverride(false).build();
        }
        ...
        if (score <= 14) {
            ...
            return PillarInterpretationDTO.builder()
                    .tier("warn").status("WARNING")
                    .text("Your DTI is " + String.format("%.0f", dti) + "% — " + String.format("%.0f", dtiAbove) + "% above the safe threshold — here is the payoff sequence that saves the most in interest.")
    ...
    private PillarInterpretationDTO computeWealth(List<PillarDTO> pillars, RawDataDTO raw) {
        ...
        if (equityPct == 0) {
            ...
            String projectedFormatted = projected >= 10000000
                    ? "₹" + formatCr(projected) + " Cr" : "₹" + formatLakh(projected) + " Lakh";
            return PillarInterpretationDTO.builder()
                    .tier("critical").status("CRITICAL")
                    ...
                    .dscrOverride(false).equityOverride(true).build();
        }
        ...
        if (score <= 14) {
            ...
            String gapFormatted = tenYearGap >= 5000000
                    ? "₹" + formatCr(tenYearGap) + " Cr" : "₹" + formatLakh(tenYearGap) + " Lakh";
            return PillarInterpretationDTO.builder()
                    .tier("warn").status("WARNING")
                    .text("Your equity exposure is " + String.format("%.0f", equityPct) + "% vs the " + String.format("%.0f", targetEquityPct) + "% target for your age — this gap costs " + gapFormatted + " in 10-year returns.")
    ...
    private PillarInterpretationDTO computeRetirement(List<PillarDTO> pillars, RawDataDTO raw) {
        ...
        if (score <= 6) {
            ...
            for (int testAge = age + 1; testAge <= 80; testAge++) {
                ...
                if (projected >= requiredCorpus) { X = testAge; break; }
                X = testAge;
            }
            ...
            double currentProjected = currentCorpus * Math.pow(1 + r, nR) + annualSavings * ((Math.pow(1 + r, nR) - 1) / r);
            ...
            return PillarInterpretationDTO.builder()
                    ...
                    .action("Increase your monthly SIP by just 10% (₹" + formatRupees(sipIncrease) + "/month more). Also consider maximising your NPS and EPF voluntary contributions for tax-efficient retirement building.")
                    .dscrOverride(false).equityOverride(false).build();
        }
        ...
        if (score <= 11) {
            ...
            return PillarInterpretationDTO.builder()
                    .tier("warn").status("WARNING")
                    .text("Your retirement corpus is ₹" + formatCr(gapAmount) + " Cr short of target — monthly SIP needs to increase by ₹" + formatRupees(additionalMonthly) + ".")
    ...
    private double getCityHealthBenchmark(String cityName) {
        List<String> metros = List.of("mumbai", "delhi", "bangalore", "bengaluru", "chennai", "kolkata", "hyderabad", "pune");
        List<String> tier1 = List.of("ahmedabad", "jaipur", "lucknow", "chandigarh", "kochi", "indore", "nagpur",
                "coimbatore", "visakhapatnam", "bhopal", "patna", "thiruvananthapuram", "gurgaon", "noida",
    ...
    private String formatCr(double v) { return String.format("%.2f", v / 10000000); }
    private String formatLakh(double v) { return String.format("%.1f", v / 100000); }
