...
class DashboardServiceTest {
    ...
    @Mock private DashboardDataLoader dataLoader;
    @Mock private HealthScoreCalculator healthScoreCalc;
    @Mock private ProjectionCalculator projectionCalc;
    @Mock private TimeMachineCalculator timeMachineCalc;
    @Mock private PriorityActionsCalculator priorityActionsCalc;
    @Mock private RedFlagsCalculator redFlagsCalc;
    @Mock private LockedInsightsCalculator lockedInsightsCalc;
    @Mock private BenchmarksCalculator benchmarksCalc;
    @Mock private ActionPlanCalculator actionPlanCalc;
    @Mock private InsuranceAnalysisCalculator insuranceAnalysisCalc;
    @Mock private TaxAnalysisCalculator taxAnalysisCalc;
    @Mock private PillarInterpretationCalculator pillarInterpretationCalc;
    ...
    private UserFinancialData stubData() {
        return UserFinancialData.builder()
                ...
                .existingLifeCover(5000000).existingHealthCover(500000)
                .lifePremium(12000).equityPct(20).savingsRate(40)
    ...
    class GetSummary {
        ...
        void shouldCallAllCalculatorsAndAssembleResponse() {
            ...
            List<PillarDTO> sortedPillars = List.of(
                    PillarDTO.builder().id("survival").score(20.0).maxScore(25).deficit(5.0).build());
            HealthScoreDTO healthScore = HealthScoreDTO.builder()
                    .totalScore(70.0).scoreLabel("GOOD").rawData(rawData)
                    .sortedPillars(sortedPillars).build();
            when(healthScoreCalc.calculate(data)).thenReturn(healthScore);

            ProjectionResultDTO projection = ProjectionResultDTO.builder().currentEndValue(1000000.0).build();
            ...
            PriorityActionsDTO priorityActions = PriorityActionsDTO.builder().actions(List.of()).build();
            when(priorityActionsCalc.calculate(data, rawData)).thenReturn(priorityActions);

            RedFlagsDTO redFlags = RedFlagsDTO.builder().flags(List.of()).totalCount(0).build();
            when(redFlagsCalc.calculate(data, rawData)).thenReturn(redFlags);

            LockedInsightsDTO lockedInsights = LockedInsightsDTO.builder().cards(List.of()).totalAvailable(0).build();
            when(lockedInsightsCalc.calculate(data, rawData)).thenReturn(lockedInsights);

            BenchmarksDTO benchmarks = BenchmarksDTO.builder().benchmarks(List.of()).build();
            when(benchmarksCalc.calculate(data, rawData)).thenReturn(benchmarks);

            ActionPlanDTO actionPlan = ActionPlanDTO.builder().actions(List.of()).build();
            when(actionPlanCalc.calculate(data, rawData)).thenReturn(actionPlan);

            InsuranceAnalysisDTO insuranceAnalysis = InsuranceAnalysisDTO.builder().age(30).build();
            when(insuranceAnalysisCalc.calculate(data, rawData)).thenReturn(insuranceAnalysis);

            TaxAnalysisDTO taxAnalysis = TaxAnalysisDTO.builder().grossTotalIncome(1200000.0).build();
            when(taxAnalysisCalc.calculate(data)).thenReturn(taxAnalysis);

            Map<String, PillarInterpretationDTO> interpretations = Map.of("survival",
        ...
        void shouldCallDataLoaderWithCorrectUserId() {
            ...
            HealthScoreDTO hs = HealthScoreDTO.builder()
                    .rawData(rawData).sortedPillars(List.of()).build();
            when(healthScoreCalc.calculate(any())).thenReturn(hs);
            when(projectionCalc.calculate(any())).thenReturn(ProjectionResultDTO.builder().build());
            when(timeMachineCalc.calculate(any(), any())).thenReturn(TimeMachineDTO.builder().build());
            when(priorityActionsCalc.calculate(any(), any())).thenReturn(PriorityActionsDTO.builder().actions(List.of()).build());
            when(redFlagsCalc.calculate(any(), any())).thenReturn(RedFlagsDTO.builder().flags(List.of()).build());
            when(lockedInsightsCalc.calculate(any(), any())).thenReturn(LockedInsightsDTO.builder().cards(List.of()).build());
            when(benchmarksCalc.calculate(any(), any())).thenReturn(BenchmarksDTO.builder().benchmarks(List.of()).build());
            when(actionPlanCalc.calculate(any(), any())).thenReturn(ActionPlanDTO.builder().actions(List.of()).build());
            when(insuranceAnalysisCalc.calculate(any(), any())).thenReturn(InsuranceAnalysisDTO.builder().build());
            when(taxAnalysisCalc.calculate(any())).thenReturn(TaxAnalysisDTO.builder().build());
        ...
        void shouldPassSortedPillarsToPillarInterpretationCalc() {
            ...
            HealthScoreDTO hs = HealthScoreDTO.builder()
                    .rawData(rawData).sortedPillars(sorted).build();
            when(healthScoreCalc.calculate(data)).thenReturn(hs);
            when(projectionCalc.calculate(any())).thenReturn(ProjectionResultDTO.builder().build());
            when(timeMachineCalc.calculate(any(), any())).thenReturn(TimeMachineDTO.builder().build());
            when(priorityActionsCalc.calculate(any(), any())).thenReturn(PriorityActionsDTO.builder().actions(List.of()).build());
