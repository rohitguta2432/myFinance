...
class PriorityActionsCalculatorTest {
    ...
    class EmergencyFund {
        ...
        void triggersWhenLow() {
            ...
            RawDataDTO raw =
                    baseRaw().emergencyFundMonths(2.0).lifeCoverRatio(1.0).build();
        ...
        void criticalWhenVeryLow() {
            ...
            var action = result.getActions().stream()
                    .filter(a -> a.getId().equals("emergency_fund"))
                    .findFirst();
            if (action.isPresent()) {
    ...
    class LifeInsurance {
        ...
        void criticalWhenSevere() {
            ...
            RawDataDTO raw = baseRaw()
                    .lifeCoverRatio(0.3)
                    .requiredCover(20000000.0)
                    .existingTermCover(6000000.0)
                    .build();
            PriorityActionsDTO result = calculator.calculate(data, raw);
            var action = result.getActions().stream()
                    .filter(a -> a.getId().equals("life_insurance"))
                    .findFirst();
            if (action.isPresent()) {
                assertThat(action.get().getUrgencyLabel()).isEqualTo("CRITICAL");
            }
    ...
    class HighEMI {
        ...
        void triggersHighEMI() {
            UserFinancialData data =
                    baseData().monthlyEMI(50000).monthlyIncome(100000).build();
            RawDataDTO raw = baseRaw().build();
            PriorityActionsDTO result = calculator.calculate(data, raw);
        ...
        void triggersModerateEMI() {
            UserFinancialData data =
                    baseData().monthlyEMI(35000).monthlyIncome(100000).build();
            RawDataDTO raw = baseRaw().build();
            PriorityActionsDTO result = calculator.calculate(data, raw);
        ...
        void doesNotTriggerLowEMI() {
            UserFinancialData data =
                    baseData().monthlyEMI(10000).monthlyIncome(100000).build();
            RawDataDTO raw = baseRaw().build();
            PriorityActionsDTO result = calculator.calculate(data, raw);
            assertThat(result.getActions())
                    .noneMatch(a -> a.getId().equals("high_emi") || a.getId().equals("moderate_emi"));
        }
    }
    ...
    class LowSavingsRate {
        ...
        void criticalWhenVeryLow() {
            UserFinancialData data =
                    baseData().savingsRate(5).monthlyIncome(100000).build();
            RawDataDTO raw = baseRaw().build();
            PriorityActionsDTO result = calculator.calculate(data, raw);
            var action = result.getActions().stream()
                    .filter(a -> a.getId().equals("low_savings"))
                    .findFirst();
            if (action.isPresent()) {
                assertThat(action.get().getUrgencyLabel()).isEqualTo("CRITICAL");
            }
    ...
    class EquityGap {
        ...
        void triggers() {
            UserFinancialData data =
                    baseData().totalAssets(1000000).equityPct(10).build();
    ...
    class TopThree {
        ...
        void maxThree() {
            // Trigger many rules: low emergency, no life insurance, no health, high EMI, low savings, low equity, low
            // retirement
